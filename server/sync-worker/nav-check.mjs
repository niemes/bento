// Broadcast nav-channel check for the sync-worker: TOFU signer pinning,
// signed control frames, read-only sockets. An INTEGRATION check, not a rig:
// it needs a live relay, so it does not live under scripts/test-* (CI runs no
// relay, and test-ci-registered rightly refuses an exemption). Run it against
// the worker before deploying a change to it:
//
//   npx wrangler dev --port 8787        (in server/sync-worker/, never through `head`)
//   node server/sync-worker/nav-check.mjs
//
// RELAY_URL overrides the target. The relay must be deployed BEFORE any client
// that depends on the broadcast control channel.
